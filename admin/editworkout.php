<?php
                  ...
                  $result=mysqli_query($conn,$sql);
                       ...
                       $id=$_REQUEST['id'];
                        ...
                        $name=$_REQUEST['name'];
                        $description=$_REQUEST['howtodo'];
                        $update="UPDATE workout set file='$image' ,workout='$topic',name='$name',howtodo='$description' where id='$id' ";
                        $result=mysqli_query($conn,$update);
                       if($result){
                        header("Location: workout.php");//redirect to view.php after successfully updated record
                         }
                        else{
  ...
  <h1 class=" w-full text-center text-grey-darkest mb-6"> Edit Gallery</h1>
    ...
    <form class="" name="form" action="" method="POST" enctype="multipart/form-data">
      ...
      <div class="flex flex-col mb-4 md:w-full">
        <label class="mb-2 uppercase  font-bold text-lg text-grey-darkest" >Select Vidoe to Upload</label>
      ...
      <div class="flex flex-col mb-4 md:w-full">
        ...
        <input class="border py-2 px-3 text-grey-darkest md:ml-2" type="text" name="workout" value="<?php echo $row['workout'];?>">
      </div>
      <div class="flex flex-col mb-4 md:w-full">
